      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div id="core-value-section" className={cn("max-w-7xl mx-auto transition-all duration-500", isVisible ? "animate-fadeIn" : "opacity-0")}>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {valuePoints.map((point, index) => <div key={index} className={cn("bg-white p-6 rounded-lg shadow-soft transition-all duration-500", isVisible ? `animate-fadeIn animation-delay-${(index + 1) * 100}` : "opacity-0")}>
          ...
          {/* Order Panel Mock */}
          <div className={cn("bg-white rounded-lg shadow-medium border border-gray-200 overflow-hidden transition-all duration-700 max-w-4xl mx-auto", isVisible ? "animate-fadeIn animation-delay-400" : "opacity-0")}>
            <div className="border-b border-gray-200 bg-gray-50 p-4">
              <h3 className="font-medium text-lg">Order Management Panel</h3>
            </div>
            
            <div className="divide-y divide-gray-200">
              {orderExamples.map((order, index) => <div key={index} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="text-gray-500">📦</div>
                    <div>
                      <div className="font-medium">Order #{order.id}</div>
                      <div className="flex items-center gap-2 text-sm">
                        <span className={cn("font-medium", order.score < 50 ? "text-red-600" : "text-green-600")}>
                          Score: {order.score}/100
                        </span>
                        <span className="text-gray-400">→</span>
                        <span className={cn("font-medium", order.status === "Flagged" ? "text-amber-600" : "text-green-600")}>
                          {order.status}
                        </span>
                        {order.reason && <>
                            <span className="text-gray-400">→</span>
                  ...
                  <div className="flex gap-2 mt-2 sm:mt-0">
                    {order.actions.map((action, actionIndex) => <Button key={actionIndex} size="sm" className={cn(action.color, "text-white")}>
                        {action.icon}
            ...
            <div className={cn("border-t border-gray-200 bg-gray-50 p-4", isVisible ? "animate-fadeIn animation-delay-500" : "opacity-0")}>
              <div className="divide-y divide-gray-200">
                <div className="py-3">
